...
The simplest case is when the electrical field is just moving in one axis. We have shown one such case in figure above, where $$\mathbf{E}$$ is pointing only in $$ \pm u_v$$. Mathematically, this can be written as 

$$\begin{align*}
E &= u_h.0.cos(\omega t-kz+\phi_h) + 
u_v E_v cos(\omega t-kz+\phi_v) \\
&= u_h E_v cos(\omega t-kz+\phi_v) 
\end{align*}$$
